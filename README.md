...
- a welcome that remains until the first interactive prompt, then dissolves away (or hides immediately with motion off), with a taller torch-shaped `π`, a layered yellow/orange flame with an anchored base, independently moving hot core, an expressive face and sparse rising/fading embers, randomized hopeful copy, and a fullscreen Settings action (use `/jar settings` in regular mode);
